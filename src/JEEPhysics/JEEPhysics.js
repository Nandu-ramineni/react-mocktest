...
function JeePhysics() {
    ...
    const [answered, setAnswered] = useState(Array(physicsData.length).fill(false));
    ...
    const handleAnswerClick = (isCorrect) => {
        ...
        if (nextQuestion < physicsData.length) {
    ...
    const handleReset = () => {
        ...
        setScore(0);
        setTimeLeft(1800);
        setAnswered(Array(physicsData.length).fill(false));
    };
    ...
    const calculatePercentage = () => {
        return (score / physicsData.length) * 100;
    };
                ...
                {showScore ? (
                    <div className="score-section">
                        {calculatePercentage() >= 35 ? (
                            <div>
                                ...
                                <p id='color'>Your final score: {score} out of {physicsData.length}</p>
                            ...
                            <div>
                                <h2>Test Failed</h2>
                                <p>Your final score: {score} out of {physicsData.length}</p>
                                <div className="progress-bar">
                                    <div className="progress" style={{ width: `${calculatePercentage()}%` }}>
                                        {calculatePercentage().toFixed(2)}%
                                    </div>
                    ...
                    <div>
                        <div className="question-info">
                            ...
                            <div className='info'><p>Question {currentQuestion + 1} of {physicsData.length}</p></div>

                        </div>
                        <Quiz
                            question={physicsData[currentQuestion].question}
                            answers={physicsData[currentQuestion].answers}
                        ...
                        <div className="navigation">
                            ...
                            <button onClick={handleNextClick} disabled={currentQuestion === physicsData.length - 1}>
                                Next Question
                            </button>
                            {currentQuestion === physicsData.length - 1 && (
